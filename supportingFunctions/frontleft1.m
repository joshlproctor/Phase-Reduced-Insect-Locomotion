function [outputset] = frontleft1(t,state,n,tdindex,actibreak)
global xffrontleft yffrontleft omega;
if tdindex==0
    outputset(1:4,1)=0;
    return;
end
datafile;
%t = t - floor(n/2)*2*pi/omega;
n=0;
...
qxfront=x-xffrontleft-((-1)^n)*d1front*cos(theta)-d2front*sin(theta);
qyfront=y-yffrontleft-((-1)^n)*d1front*sin(theta)+d2front*cos(theta);
