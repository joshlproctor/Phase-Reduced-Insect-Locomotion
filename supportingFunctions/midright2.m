function [outputset] = midright2(t,state,n,tdindex,actibreak)
global xfmidright yfmidright omega;
if tdindex==0
    outputset(1:4,1)=0;
    return;
end
...
qxmid=x-xfmidright-((-1)^(n+1))*d1mid*cos(theta)-d2mid*sin(theta);
qymid=y-yfmidright-((-1)^(n+1))*d1mid*sin(theta)+d2mid*cos(theta);
