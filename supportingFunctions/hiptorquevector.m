function torque = hiptorquevector(time,jointangle,jointrate,num,actfun)
global omega vmax moment_arm tors_k cdamp highvmax Vdes muscSn FreqCheck SpringCheck;
global tors_k cdamp
% global pertarray randspike expno param;
...
%% Hill-type muscle
% par=param((num+3),:) + pertarray((num+3),:).*(randspike==1);

steep=1; swing=1;
time=time.*1000;

if swing==1
    extsig=1+exp( steep.*(time-1000*pi/omega) );
    flexsig=1 + exp( -steep.*(time-1000*pi/omega) ).*(num==3);
end
...
rest_angle = 0.7*(num==1) + 0.75*(num==2) + 0.7*(num==3);
...
% if Vdes==.26
%  par = muscSn(1:2,1)*(num==1) + muscSn(5:6,1)*(num==2) + muscSn(9:10,1)*(num==3);
% end
FreqCheck = 0;
 if FreqCheck==0
 par = muscS(1:2,1)*(num==1) + muscS(5:6,1)*(num==2) + muscS(9:10,1)*(num==3);
 end
 if FreqCheck==1
     par = muscSn(1:2,1)*(num==1) + muscSn(5:6,1)*(num==2) + muscSn(9:10,1)*(num==3);
 end
...
% if Vdes~=.26
% 
% FrontmuscSe=5/.6*Vdes;
% FrontmuscSf=3/.6*Vdes+1;
% 
% MidmuscSe=18/.6*Vdes+2;
% MidmuscSf=2/.6*Vdes+1;
% 
% RearmuscSe=3.5/.6*Vdes+2;
% RearmuscSf=1/.6*Vdes+1.5;
% 
