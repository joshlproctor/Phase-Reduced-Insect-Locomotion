function [outputset] = hindleft3(t,state,n,tdindex,actibreak)
global xfrearleft yfrearleft omega;
if tdindex==0
    outputset(1:4,1)=0;
    return;
end
...
qxrear=x-xfrearleft-((-1)^n)*d1rear*cos(theta)-d2rear*sin(theta);
qyrear=y-yfrearleft-((-1)^n)*d1rear*sin(theta)+d2rear*cos(theta);
